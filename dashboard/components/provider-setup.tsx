...
export default function ProviderSetup({ provider, onClose, onSaved }: { provider?: GatewayRow; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      validate();
      const settings = JSON.parse(providerSettings || "{}");
      if (!settings || typeof settings !== "object" || Array.isArray(settings)) throw new Error();
      setBusy(true);
  ...
  return <div className="dialog-backdrop"><section ref={dialog} className="editor provider-setup" role="dialog" aria-modal="true" aria-labelledby="provider-setup-title" aria-busy={loading || busy}>
    <div className="editor-head"><div><h2 id="provider-setup-title">{editing ? `Configure ${text(provider?.name)}` : "Add provider"}</h2><p>Reconcile the provider, credentials, canonical models, mappings, routes, and credential pool.</p></div><button type="button" className="icon-button" onClick={onClose} disabled={busy} aria-label="Close provider setup"><X size={18} /></button></div>
    <form onSubmit={submit}>{loading || (editing && hydrationFailed) ? <div className="loading-state" role={hydrationFailed ? "alert" : "status"}>{hydrationFailed ? <span>{error}. Provider configuration could not be loaded. Close and retry.</span> : "Loading provider configuration..."}</div> : <>
      <fieldset disabled={busy}><legend>Provider</legend><div className="provider-grid">
        ...
        <label className="field full"><span>Provider settings JSON</span><textarea value={providerSettings} onChange={(event) => setProviderSettings(event.target.value)} rows={4} /></label>
      </div></fieldset>
      <fieldset disabled={busy}><legend>Credentials</legend><div className="subsection-head"><p>Existing labels are immutable. Rotation requires an explicit selection and a new secret.</p><button type="button" onClick={() => setCredentials((items) => [...items, emptyCredential()])}><Plus size={14} />Credential</button></div>
        <div className="repeater-list">{credentials.map((item, index) => <div className="repeater" key={index}>
          ...
          <label className="field"><span>Priority</span><input type="number" min="0" step="1" value={item.priority} onChange={(event) => updateCredential(index, { priority: event.target.value })} /></label>
          <label className="field"><span>Requests/minute</span><input type="number" min="1" step="1" value={item.requests_per_minute} onChange={(event) => updateCredential(index, { requests_per_minute: event.target.value })} /></label>
          <label className="field"><span>Tokens/minute</span><input type="number" min="1" step="1" value={item.tokens_per_minute} onChange={(event) => updateCredential(index, { tokens_per_minute: event.target.value })} /></label>
          <label className="field"><span>Quota limit</span><input type="number" min="0" step="any" value={item.quota_limit} onChange={(event) => updateCredential(index, { quota_limit: event.target.value })} /></label>
          <label className="field"><span>Quota threshold</span><input type="number" min="0.01" max="1" step="0.01" value={item.quota_threshold} onChange={(event) => updateCredential(index, { quota_threshold: event.target.value })} /></label>
      ...
      <fieldset disabled={busy}><legend>Models and routes</legend><div className="subsection-head"><p>Each mapping retains its complete transport settings and pricing metadata.</p><button type="button" onClick={() => setMappings((items) => [...items, emptyMapping()])}><Plus size={14} />Mapping</button></div>
        <div className="mapping-list">{mappings.map((item, index) => <article className="mapping-card" key={index}>
          ...
          <div className="provider-grid"><label className="field"><span>Canonical model ID</span><input value={item.model_id} onChange={(event) => updateMapping(index, { model_id: event.target.value })} required /></label><label className="field"><span>Display name</span><input value={item.display_name} onChange={(event) => updateMapping(index, { display_name: event.target.value })} required /></label>
          ...
          <label className="field"><span>Upstream model ID</span><input value={item.upstream_model_id} onChange={(event) => updateMapping(index, { upstream_model_id: event.target.value })} required /></label><label className="field"><span>Protocol</span><select value={item.protocol} onChange={(event) => updateMapping(index, { protocol: event.target.value })}>{protocols.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
          <label className="field"><span>Mapping priority</span><input type="number" min="0" step="1" value={item.priority} onChange={(event) => updateMapping(index, { priority: event.target.value })} /></label><label className="field"><span>Weight</span><input type="number" min="0.001" step="any" value={item.weight} onChange={(event) => updateMapping(index, { weight: event.target.value })} /></label><label className="field"><span>Maximum concurrency</span><input type="number" min="1" step="1" value={item.max_concurrency} onChange={(event) => updateMapping(index, { max_concurrency: event.target.value })} /></label>
          ...
          <details><summary>Transport settings</summary><label className="field"><span>Settings JSON</span><textarea value={item.settings_json} onChange={(event) => updateMapping(index, { settings_json: event.target.value })} rows={4} /></label></details>
          <div className="pricing-grid"><label className="field"><span>Input / million</span><input type="number" min="0" step="any" value={item.input_price} onChange={(event) => updateMapping(index, { input_price: event.target.value })} /></label><label className="field"><span>Output / million</span><input type="number" min="0" step="any" value={item.output_price} onChange={(event) => updateMapping(index, { output_price: event.target.value })} /></label><label className="field"><span>Cached input / million</span><input type="number" min="0" step="any" value={item.cached_price} onChange={(event) => updateMapping(index, { cached_price: event.target.value })} /></label><label className="field"><span>Currency</span><input maxLength={3} value={item.currency} onChange={(event) => updateMapping(index, { currency: event.target.value })} /></label></div>
          <div className="route-controls"><label className="check-control"><input type="checkbox" checked={item.route_present} onChange={(event) => updateMapping(index, { route_present: event.target.checked })} />Include route</label><label className="check-control"><input type="checkbox" checked={item.route_enabled} disabled={!item.route_present} onChange={(event) => updateMapping(index, { route_enabled: event.target.checked })} />Route enabled</label><label className="check-control"><input type="checkbox" checked={item.allow_model_fallback} disabled={!item.route_present} onChange={(event) => updateMapping(index, { allow_model_fallback: event.target.checked })} />Allow model fallback</label><label className="field"><span>Route priority</span><input type="number" min="0" step="1" disabled={!item.route_present} value={item.route_priority} onChange={(event) => updateMapping(index, { route_priority: event.target.value })} /></label></div>
        </article>)}</div>
      </fieldset>
